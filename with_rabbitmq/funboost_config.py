...
class BrokerConnConfig(DataClassBase):
    ...
    # 修改RabbitMQ配置为Docker中的配置
    RABBITMQ_USER = 'admin'
    RABBITMQ_PASS = 'admin123'
    RABBITMQ_HOST = '127.0.0.1'
    RABBITMQ_PORT = 5672
    RABBITMQ_VIRTUAL_HOST = ''  # 使用默认虚拟主机
    ...
    REDIS_PORT = 6379
    REDIS_DB = 0  # redis消息队列所在db，请不要在这个db放太多其他键值对，框架里面有的功能会scan扫描unacked的键名，使用单独的db。
...
class FunboostCommonConfig(DataClassBase):
    ...
    NB_LOG_FORMATER_INDEX_FOR_CONSUMER_AND_PUBLISHER = logging.Formatter(
        f'%(asctime)s-%(name)s-"%(filename)s:%(lineno)d"-%(funcName)s-%(levelname)s-%(task_id)s-%(message)s',
        "%Y-%m-%d %H:%M:%S")
    
    # 设置总体日志级别（这会影响大多数日志输出）
    ...
    # 消费者日志级别
    CONSUMER_LOGGER_LEVEL = logging.INFO
    
    # 发布者日志级别
    PUBLISHER_LOGGER_LEVEL = logging.INFO
    
    # 其他组件的日志级别
    PARAMS_CHECKER_LOGGER_LEVEL = logging.WARNING  # 参数检查器的日志级别
    DECORATOR_LOGGER_LEVEL = logging.WARNING      # 装饰器的日志级别
    BROKER_LOGGER_LEVEL = logging.WARNING         # 消息中间件的日志级别
    CONCURRENT_LOGGER_LEVEL = logging.WARNING     # 并发模块的日志级别
